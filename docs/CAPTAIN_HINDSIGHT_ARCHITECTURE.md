...
- **The private repo.** Create `JohnnyM77/Reporting-Agent-private`, add a fine-grained token with contents read/write on it as the `HINDSIGHT_PRIVATE_REPO_TOKEN` secret, and set the repo variable `HINDSIGHT_STORE=private_repo`. Until then, scheduled runs use `local` on the runner: they still email, but nothing persists between runs, so dedup, queueing, triage baselines and autopsies do not carry over.
