...
def _html_event_card(event: InvestorEvent, priority_color: str) -> str:
    ...
    rows = [
        ...
        f'    <span style="background:{priority_color};color:#fff;font-size:11px;'
        f'font-weight:700;padding:2px 8px;border-radius:4px;">'
        f'{htmlmod.escape(event.priority)}</span>',
        f'    <span style="color:{_COLOR_TEXT};font-weight:700;font-size:15px;">'
        f'{ticker_display}</span>',
        f'    <span style="color:{_COLOR_MUTED};font-size:13px;">{company_display}</span>',
        f'    <span style="color:{_COLOR_MUTED};font-size:11px;margin-left:auto;">'
        f'Score: {event.score}</span>',
        f'  </div>',
        f'  <div style="color:{_COLOR_TEXT};font-size:14px;font-weight:600;margin-bottom:4px;">'
        f'{headline_display}</div>',
    ]
    if summary_display:
        rows.append(
            f'  <div style="color:{_COLOR_MUTED};font-size:13px;margin-bottom:4px;">'
            f'{summary_display}</div>'
        )
    if thesis_display:
        rows.append(
            f'  <div style="color:{_COLOR_MUTED};font-size:12px;font-style:italic;'
            f'margin-bottom:4px;">Thesis impact: {thesis_display}</div>'
        )
    if action_display:
        rows.append(
            f'  <div style="color:{_COLOR_HIGH};font-size:13px;font-weight:600;'
            f'margin-bottom:4px;">Action: {action_display}</div>'
        )
    if links_html:
        rows.append(
            f'  <div style="font-size:12px;margin-top:4px;">{links_html}</div>'
        )
    rows.append(f'</td></tr>')
    return "\n".join(rows)
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:700px;margin:0 auto;">
  ...
  {content}
  <tr>
    <td style="padding:16px;color:{_COLOR_MUTED};font-size:11px;text-align:center;
