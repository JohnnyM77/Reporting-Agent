...
def main() -> None:
    ...
    # Reuse Bob's exact Drive folder-id pattern first; keep Sally var as fallback.
    drive_folder_id = os.environ.get("GDRIVE_FOLDER_ID") or os.environ.get("SALLY_DRIVE_ROOT_FOLDER_ID")
    now_local = dt.datetime.now(ZoneInfo(tz))
    run_label = f"SundaySally_{now_local.date().isoformat()}"
    drive_link = upload_run_folder(run_folder, folder_id=drive_folder_id or "", run_label=run_label)
